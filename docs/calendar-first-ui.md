...
- Use Gradio 6.27's available `gr.Sidebar(position="right", width=440, open=False)` primitive with `elem_id="jf-compose-panel"` as the sole approved composition primitive; do not substitute an inline collapsible panel, custom modal, or always-visible column.
...
- The owned native Sidebar path is required throughout the supported Gradio range. There is no approved inline-panel or custom-modal fallback; failure of fixed geometry, focus, ARIA, or overflow acceptance checks blocks implementation until corrected on the Sidebar path.
