...
- Domain/source IDs are stable, UI-safe strings and use the character rule `^[A-Za-z0-9][A-Za-z0-9_-]*$`. `DeadlineTask.id` and `RecurringRoutine.id` are work-item IDs and must contain at most 48 characters because they are embedded verbatim in generated block IDs. Every other domain/source ID contains at most 64 characters. Extraction creates deterministic IDs in input order (`task-01`, `routine-01`, `fixed-01`); edits preserve them. Presentation-layer derived IDs, including `CalendarEventView.view_id`, use the same character rule but have the separate exact bounds in section 8.1; they are not persisted/domain IDs and do not change their unmodified `source_id`.
- A generated schedule-block ID is exactly `block-{work_id}-{sequence:02d}`: `sequence` is decimal, starts at 1 independently for each work item, has a minimum width of two digits (`01` through `99`) and expands normally at `100` and `1000`; neither `work_id` nor the ID may be truncated, hashed, or rewritten. A 31-day month contains `31 * 48 = 1,488` grid slots. Because generated blocks cannot overlap, the entire result and any one work item can contain at most 1,488 blocks. The largest sequence is therefore `1488` (four digits). A 48-character work-item ID yields at most `6 + 48 + 1 + 4 = 59` characters (`block-` is six characters), within the 64-character domain-ID ceiling. Tests must exercise the four-digit sequence boundary even if normal daily caps produce fewer blocks.
...
1. Domain/source IDs are unique across tasks, routines, availability, and fixed events. Validation deterministically rejects a `DeadlineTask.id` or `RecurringRoutine.id` longer than 48 characters at model construction, before scheduling, and rejects every other domain/source ID longer than 64 characters. Generated block IDs retain the exact `block-{work_id}-{sequence:02d}` form and are also validated against the 64-character domain-ID ceiling. UI projection IDs are validated separately against section 8.1 and are never accepted as substitutes for source IDs.
...
- Each generated block becomes one event segment because generated blocks cannot cross local midnight. A fixed event that intersects the month is clipped to the month and split at local midnights into one segment per selected-month date. Its `view_id` is exactly `fixed-view-{fixed_event_id}-{YYYYMMDD}`; a generated block uses exactly `view_id=ScheduleBlock.id`.
- `CalendarEventView.view_id` is a presentation-layer derived identifier, not a persisted/domain ID. Both formulas satisfy `^[A-Za-z0-9][A-Za-z0-9_-]*$` and are collision-free within the rendered month: schedule-block IDs are unique, each fixed-event segment key combines a unique fixed-event ID with its unique local segment date, and the `block-` and `fixed-view-` namespaces are disjoint. The generated-block form is at most 59 characters. The fixed-event form is at most `11 + 64 + 1 + 8 = 84` characters (`fixed-view-` is 11 characters), so 84 is the exact overall `view_id` ceiling. Implementations must preserve `source_id` verbatim and must not truncate, hash, rewrite, or otherwise shorten either identifier. Re-rendering identical confirmed input and result must reproduce byte-identical `view_id` values.
...
- **QA end to end**: clean keyless install; pytest, coverage, Ruff, mypy, and pip integrity; canonical fixture through real loopback Gradio; February leap/non-leap and 30/31-day probes; outside-horizon matrix; deterministic repeat; month change invalidation; independent max-boundary checks for both calendar `view_id` formulas (59-character generated block and 84-character fixed-event segment), regex compliance, cross-segment uniqueness, unmodified `source_id`, and repeated-render stability; responsive screenshots at representative desktop/mobile widths; DOM/config/accessibility inspection; no provider traffic, secret leakage, persistence, or unsupported optimality claim.
